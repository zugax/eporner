...
const WORKER_BASE = process.env.NEXT_PUBLIC_API_BASE || EPORNER_API_BASE
const EPORNER_PATH = '/api/eporner'

// Helper to get the correct API path
const getApiPath = (path) => WORKER_BASE.includes('localhost') ? `${EPORNER_PATH}${path}` : path
...
export async function getVideos(options = {}) {
  ...
  let endpoint = getApiPath('/video/search')
...
export async function getVideoById(id) {
  ...
  const searchConfigs = [
    ...
    { url: `${WORKER_BASE}${getApiPath('/video/search')}?per_page=200&thumbsize=big&format=json`, cacheKey: `video-latest-${id}` },
    // Second: search in trending
    { url: `${WORKER_BASE}${getApiPath('/video/search')}?per_page=100&thumbsize=big&order=trending&format=json`, cacheKey: `video-trending-${id}` },
    // Third: search in popular
    { url: `${WORKER_BASE}${getApiPath('/video/search')}?per_page=100&thumbsize=big&order=popular&format=json`, cacheKey: `video-popular-${id}` },
    // Fourth: try search with ID as query (some videos may match)
    { url: `${WORKER_BASE}${getApiPath('/video/search')}?query=${id}&per_page=40&thumbsize=big&format=json`, cacheKey: `video-query-${id}` },
  ]
  ...
  try {
    const idUrl = `${WORKER_BASE}${getApiPath('/video/id/' + id)}?thumbsize=big&format=json`
...
export async function getCategories() {
  ...
  const endpoints = [
    getApiPath('/video/categories'),
    getApiPath('/categories'),
    getApiPath('/video/cats')
  ]
...
export async function getPornstars(page = 1, perPage = 30) {
  ...
  const endpoints = [
    getApiPath(`/pornstar/list?page=${page}&per_page=${perPage}`),
    getApiPath(`/pornstars?page=${page}&per_page=${perPage}`),
    getApiPath(`/pornstar/featured?page=${page}&per_page=${perPage}`)
  ]
...
export async function getPornstarById(id) {
  ...
  const endpoints = [
    getApiPath(`/pornstar/id/${id}`),
    getApiPath(`/pornstars/${id}`),
    getApiPath(`/pornstar/${id}`)
  ]
